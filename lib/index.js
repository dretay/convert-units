  ...
  */
  each(this.possibilities(), function(possibility) {
    ...
    if (isIncluded && unit.system === this.origin.system) {
      ...
      function is_best(reverse, maxNumber, result){
        if(!reverse){
          return (result < best.val);
        }
        else{
          return (result > best.val) && result < maxNumber
        }
      }
      if (!best || (result >= options.cutOffNumber && is_best(options.reverse, options.maxNumber, result))) {
        best = {
          val: result,
          unit: possibility,
